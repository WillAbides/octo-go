...
func run(schemaPath, outputPath, pkgPath, pkgName string) error {
	...
	err = renderRequestFiles(reqFiles, outputPath, pkgPath)
	if err != nil {
		return err
	}
...
func renderRequestFiles(reqFiles map[string]*jen.File, outputPath, pkgPath string) error {
	for pkg, reqFile := range reqFiles {
		...
		var hf *os.File
		hf, err = os.Create(filepath.Join(dir, "helper_gen.go"))
		if err != nil {
			return err
		}
		helperContent, err := reqHelperFile(pkg, pkgPath)
		if err != nil {
			return err
		}
		_, err = hf.Write(helperContent)
		if err != nil {
			return err
		}
	}
