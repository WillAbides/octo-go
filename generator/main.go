...
func run(schemaPath, outputPath, pkgPath, pkgName string) error {
	...
	internalAttrsOut, err := os.Create(filepath.Join(internalOutputPath, "endpoint_attrs_gen.go"))
	if err != nil {
		return err
	}
	internalAttrsFile := jen.NewFilePathName(internalPkgPath, internalPkgName)
	internalAttrsFile.HeaderComment("Code generated by octo-go; DO NOT EDIT.")
	addEndpointAttributes(internalAttrsFile, endpoints)
	err = internalAttrsFile.Render(internalAttrsOut)
	if err != nil {
		return err
	}
